...
function compareArrays(arr1, arr2) {
  ...
  const sortedArr = [arr1, arr2].sort((a, b) => a.length - b.length);
...
function traceBackward(n) {
  ...
  console.log(n);
  n > 1 ? traceBackward(n - 1) : console.trace(n);
}
...
function countVowelsAndConsonants(word) {
  ...
  for (const letter of word.toLowerCase()) {
    if (vowels.includes(letter)) {
      vowelCount++;
      console.info(`Голосних: ${vowelCount}`);
    } else {
      consonantCount++;
      console.info(`Приголосних: ${consonantCount}`);
    }
  }
}
